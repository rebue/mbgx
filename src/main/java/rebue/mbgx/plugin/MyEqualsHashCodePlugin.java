...
/**
 * <pre>
 * 为Model类自动生成equals和hashCode方法
 * 重写了原生的插件，复制的是1.3.5版本的源码
 * 原生的插件对所有属性都加入到equals和hashCode方法中有问题
 * 修改为只根据id就可以了
 * </pre>
 * 
 * @author zbz
 * 
 */
public class MyEqualsHashCodePlugin extends PluginAdapter {
    ...
    protected void generateEquals(TopLevelClass topLevelClass, List<IntrospectedColumn> introspectedColumns,
        ...
        while (iter.hasNext()) {
            ...
            // XXX MBG : MyEqualsHashCodePlugin : FIX-判断必须是主键
            // 目前1.3.5版本的mbg有bug，不能通过isIdentity来判断是否是主键
            // if (!introspectedColumn.isIdentity())
            if (!isPrimaryKey(introspectedColumn, introspectedTable))
                continue;
    ...
    private boolean isPrimaryKey(IntrospectedColumn introspectedColumn, IntrospectedTable introspectedTable) {
