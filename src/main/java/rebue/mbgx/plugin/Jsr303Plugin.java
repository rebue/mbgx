    ...
    public boolean modelFieldGenerated(final Field field, final TopLevelClass topLevelClass,
        ...
        if (!introspectedColumn.isNullable()) {
            ...
            final String groups = "groups = "
                    + (IntrospectedUtils.isPrimaryKey(introspectedColumn, introspectedTable) ? "ModifyGroup.class"
                            : "{AddGroup.class, ModifyGroup.class}");
            // 如果是字符串类型，添加 @NotBlank 注解
            if (introspectedColumn.isStringColumn()) {
                topLevelClass.addImportedType("javax.validation.constraints.NotBlank");
                field.addAnnotation("@NotBlank(" + groups + ", message = \""
                        + RemarksUtils.getTitleByRemarks(introspectedColumn.getRemarks()) + "不能为空\")");
            }
            // 如果是非字符串类型，添加 @NotNull 注解
            else {
                topLevelClass.addImportedType("javax.validation.constraints.NotNull");
                field.addAnnotation("@NotNull(" + groups + ", message = \""
                        + RemarksUtils.getTitleByRemarks(introspectedColumn.getRemarks()) + "不能为空\")");
            }
        }
        ...
        if (introspectedColumn.isStringColumn() && introspectedColumn.getLength() > 0) {
            ...
            field.addAnnotation("@Length(max = " + introspectedColumn.getLength() + ", message = \""
                    + RemarksUtils.getTitleByRemarks(introspectedColumn.getRemarks()) + "的长度不能大于"
        ...
        if (introspectedColumn.getActualTypeName().contains("UNSIGNED")) {
            ...
            field.addAnnotation("@PositiveOrZero(message = \""
                    + RemarksUtils.getTitleByRemarks(introspectedColumn.getRemarks()) + "不能为负数\")");
        }
